...
def test_find_files_matches_glob(tmp_path) -> None:
    ...
    deps = CommitToolDeps(git_adapter=adapter, commit_sha=sha)
...
def test_search_text_at_repo_root_without_glob_rejected(tmp_path) -> None:
    adapter, sha = _make_repo(tmp_path)
    deps = CommitToolDeps(git_adapter=adapter, commit_sha=sha, requirement_id="FR-3")
...
def test_search_text_caps_files_scanned(tmp_path, monkeypatch) -> None:
    adapter, sha = _make_repo(tmp_path)
    deps = CommitToolDeps(git_adapter=adapter, commit_sha=sha)
    monkeypatch.setenv("IMPLEMENTS_AGENT_SEARCH_MAX_FILES", "1")
...
def test_read_file_rejects_oversized_without_line_range(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("IMPLEMENTS_AGENT_READ_MAX_BYTES", "100")
    path = tmp_path / "repo"
    path.mkdir()
    repo = init_repo_with_config(path)
    rename_default_branch_to_main(repo)
...
def test_read_file_rejects_wide_line_span(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("IMPLEMENTS_AGENT_READ_MAX_LINE_SPAN", "50")
    adapter, sha = _make_repo(tmp_path)
    deps = CommitToolDeps(git_adapter=adapter, commit_sha=sha)
...
def test_read_file_tool_cache_reuses_full_file_read(tmp_path) -> None:
    ...
    deps = CommitToolDeps(git_adapter=adapter, commit_sha=sha, tool_cache=cache)
...
def test_read_file_logs_start_before_hang_risk(tmp_path, monkeypatch, caplog) -> None:
    ...
    adapter, sha = _make_repo(tmp_path)
    deps = CommitToolDeps(git_adapter=adapter, commit_sha=sha, requirement_id="FR-2")
    caplog.set_level(logging.INFO)
