...
from specvsreality_worker.agents.artifact_candidate_agent.glob import (
    files_under_prefix,
    glob_match,
    normalize_relpath,
)
...
def _execute_tool(
    ...
    logger.info(
        "tool %s start label=%s commit=%s %s",
        tool_name,
        _label(deps),
        deps.commit_sha[:7],
        detail,
    ...
    if elapsed_s > 5.0:
        logger.warning(
            "tool %s slow label=%s commit=%s elapsed_s=%.2f %s",
            tool_name,
            _label(deps),
            deps.commit_sha[:7],
            elapsed_s,
...
def _run_cached_tool(
    ...
    if cache is not None and cache.has(cache_key):
        logger.info(
            "tool %s cache_hit label=%s commit=%s %s",
            tool_name,
            _label(deps),
            deps.commit_sha[:7],
            detail,
...
def find_files(deps: CommitToolDeps, glob: str) -> list[str]:
    ...
    def _run() -> list[str]:
        ...
        cap = deps.settings.artifact_candidate_agent_find_files_max
        truncated = len(matches) > cap
        if truncated:
            matches = matches[:cap]

        logger.info(
            "tool find_files done label=%s commit=%s glob=%r matches=%s "
...
def list_directory(deps: CommitToolDeps, path: str) -> list[str]:
    ...
    def _run() -> list[str]:
        ...
        try:
            ...
            return [f"error: {exc}"]
        logger.info(
            "tool list_directory done label=%s commit=%s path=%r children=%s "
            "elapsed_s=%.2f",
            _label(deps),
            deps.commit_sha[:7],
            path,
...
def read_file(
    ...
    def _run() -> str:
        started = time.monotonic()
        max_bytes = deps.settings.artifact_candidate_agent_read_max_bytes
        try:
        ...
        if line_range:
            ...
            span = end - start + 1
            max_span = deps.settings.artifact_candidate_agent_read_max_line_span
            if span > max_span:
                return (
            ...
            try:
                ...
                return f"error: {exc}"
            logger.info(
                "tool read_file done label=%s commit=%s path=%r bytes=%s "
        ...
        if not lines:
            logger.info(
                "tool read_file done label=%s commit=%s path=%r empty elapsed_s=%.2f",
                _label(deps),
                deps.commit_sha[:7],
                path,
                time.monotonic() - started,
        ...
        max_lines = deps.settings.artifact_candidate_agent_read_max_lines
...
def search_text(
    ...
    def _run() -> list[str]:
        ...
        if error is not None:
            logger.info(
                "tool search_text done label=%s commit=%s rejected %s",
                _label(deps),
                deps.commit_sha[:7],
                error,
            )
        ...
        max_files = deps.settings.artifact_candidate_agent_search_max_files
        max_matches = deps.settings.artifact_candidate_agent_search_max_matches
        max_bytes = deps.settings.artifact_candidate_agent_read_max_bytes
        ...
        elapsed_s = time.monotonic() - started
        logger.info(
            "tool search_text done label=%s commit=%s pattern=%r path=%r file_glob=%r "
        ...
        if elapsed_s > 5.0:
            logger.warning(
                "tool search_text slow label=%s commit=%s elapsed_s=%.2f",
                _label(deps),
                deps.commit_sha[:7],
                elapsed_s,
            )
